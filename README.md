...
The ORForise platform is written in Python3 and only requires the Numpy library which is standard in most base installations of Python3. 
...
Genome annotation is a difficult process, 'even for Prokaryotes'. ORForise allows the direct and systematic analysis 
of a novel ORF prediction from a wide selection of tools to a Gold Standard Genome Annotation, such as those provided
by Ensembl Bacteria.
...
genes plus the new ORFs from another prediction. Default filtering to remove additional ORFs that overlap existing genes by 
more than 50 nt. 
...
### GFF_Intersector:
GFF_Intersector enables the aggregation of different genome annotations/ORF predictions and creates a single 
GFF representing the intersection of the two existing GFFs. 

GFF_Intersector also provides an option to allow the retention of genes that have a user defined difference (minimum 75\% coverage 
and in-frame).
